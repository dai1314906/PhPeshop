...
<div id="container">
    ...
    <div  class="wrapper">
        ...
        <div>
            ...
            include ("conn.php");
            $id = $_GET['id'];
            $sql = mysqli_query($link,"select * from tb_user join tb_consult on tb_consult.uid = tb_user.uid join tb_commodity on tb_commodity.cid = tb_consult.cid where tb_consult.conid = '$id'");
            $row = mysqli_fetch_array($sql);
            ?>
        <p style="text-align: center;">商品:<?php echo $row['cname']?>&nbsp;&nbsp;用户:<?php echo $row['username'];?>&nbsp;&nbsp;&nbsp;&nbsp;用户评价内容:<?php echo $row['concontent']?>&nbsp;&nbsp;&nbsp;&nbsp;用户评价时间:<?php echo $row['contime']?></p>
        </div>
        <div>
        <form action="retextuser.php" method="post">
            <textarea name="retextuser" id="retextuser" cols="30" rows="10">
            </textarea>
            <br>
            <input type="hidden" name="conid" id="conid" value="<?php echo $row['conid']?>">
            <button type="submit" name="Submit" class="btn btn_lg fontcolor">回复</button>
